...
def normalize_and_verify(rows:List[Dict[str,any]])->pd.DataFrame:
    ...
    for r in rows:
        ...
        if marks_count is not None and last_col is not None:
            verified = "Yes" if marks_count == last_col else "No"; final_tally = last_col
...
def call_gemini_vision_for_cop(image_bytes:bytes,mime_type:str)->Dict[str,any]:
    ...
    prompt="Extract Cop Winding (Weft) register data as strict JSON with header and rows."
    try:
...
with colL:
    ...
    if reg!="Cop Winding (Weft)":
        ...
        if img_bytes:
            ...
            if len(bad_lots): st.warning(f"⚠️ {len(bad_lots['Quality'].unique())} lot(s) failed lot footer verification.")
            ...
            final_cols=["Sl_No","Frame_No","Worker_Name","Labour_No","Quality",
            ...
            c2.download_button("⬇️ XLSX",xlsx_buf.getvalue(),
                               "cop_winding_clean.xlsx",
                               "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.caption("💡 Note: Only the selected key columns are saved/exported. PDF removed; XLSX added.")
        else:
            st.info("📷 Capture or Upload an image to begin")

with colR:
    st.subheader("2-Step Tally Verification")
    st.write("- Step A: Count from marks — `|=1`, `||=2`, `|||=3`, `||||=4`, `||||/=5`\n"
